...
// Designed for BlogEngine.NET version 2.5, .NET Framework 4. A version for 
// BlogEngine.NET version 2.0 is available at http://www.johnpcrepair.com/blog/post/2011/10/28/ExcerptImages-Extension-for-BlogEngineNET.aspx
/// <summary>
/// Automatically places the first image from a post into the post list excerpt or
/// another picture if specified
/// </summary>
[Extension("Automatically places the first image from a post into the post list excerpt. Requires BlogParser 1.1 or higher.", "1.0.0.1", "John Thomas")]
public class ExcerptImages : BlogParser.ISubscriber
{
    ...
    private static void UpdateSettings()
    {
        try
        {
            ...
            _ImageSize = new ImageInfo(_MaxWidth, _MaxHeight, _Units);
    ...
    private static ImageInfo GetSmallFile(string bigFile)
    {
        ...
        if (!File.Exists(imageUri.SmallFileHostPath)) // See if the small file already exists
        {
            ...
            {
                return new ImageInfo() { File = "\0" + ShowError(err, "DirectoryNotFound: " + imageUri.FileHostPath) };
            }
            catch (FileNotFoundException err)
            {
                return new ImageInfo() { File = "\0" + ShowError(err, "FileNotFound: " + imageUri.FileHostPath) };
            }
            catch (PathTooLongException err)
            {
                return new ImageInfo() { File = "\0" + ShowError(err, "PathTooLong: " + imageUri.FileHostPath) };
            }
            catch (IOException err)
            {
                return new ImageInfo() { File = "\0" + ShowError(err, "IOException: " + imageUri.FileHostPath) };
            }
            catch (Exception ex)
            {
                return null;
            ...
            {
                return new ImageInfo() { File = "\0" + ShowError(err, "FileNotFound:GetSmallFile:" + imageUri.FileHostPath) };
            }
            catch (DirectoryNotFoundException err)
            {
                return new ImageInfo() { File = "\0" + ShowError(err, "DirectoryNotFound:GetSmallFile:" + imageUri.FileHostPath) };
            }
            catch (PathTooLongException err)
            {
                return new ImageInfo() { File = "\0" + ShowError(err, "PathToLong:GetSmallFile:" + imageUri.FileHostPath + ", " + imageUri.SmallFileHostPath) };
            }
        }
        else
        {
    ...
    private static ImageInfo GetDisplaySize(float origWidth, float origHeight)
    {
        float ratio;
        ...
        return new ImageInfo(Convert.ToInt32(goalWidth), Convert.ToInt32(goalHeight), _Units);
    }
    ...
    public class ImageInfo
    {
        ...
        public ImageInfo(string width, string height)
        {
            Width = width;
            Height = height;
        }
        ...
        public ImageInfo(int width, int height, string units)
        {
